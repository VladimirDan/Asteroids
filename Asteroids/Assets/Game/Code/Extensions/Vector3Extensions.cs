using UnityEngine;

namespace Game.Scripts.Extensions
{
    public static class Vector3Extensions
    {
        public static Vector3 Inverse(this Vector3 self)
        {
            self.x = Mathf.Approximately(self.x, 0) ? 1 : 1 / self.x;
            self.y = Mathf.Approximately(self.y, 0) ? 1 : 1 / self.y;
            self.z = Mathf.Approximately(self.z, 0) ? 1 : 1 / self.z;
            return self;
        }

        public static Vector3 Direction(Vector3 start, Vector3 end)
            => (end - start).normalized;

        public static Vector3 GetScaled(this Vector3 self, Vector3 other1)
        {
            self.Scale(other1);
            return self;
        }

        public static Vector3 GetScaled(this Vector3 self, Vector3 other1, Vector3 other2)
        {
            self.Scale(other1);
            self.Scale(other2);
            return self;
        }

        public static Vector3 GetScaled(this Vector3 self, Vector3 other1, Vector3 other2, Vector3 other3)
        {
            self.Scale(other1);
            self.Scale(other2);
            self.Scale(other3);
            return self;
        }

        public static bool IsVisible(this Vector3 self)
        {
            return 0 < Mathf.Abs(self.x * self.y * self.z);
        }

        public static Vector3 GetIncreased(this Vector3 self, float value)
        {
            self.x += value;
            self.y += value;
            self.z += value;

            return self;
        }

        public static Vector3 GetIncreasedX(this Vector3 self, float value)
        {
            self.x += value;

            return self;
        }

        public static Vector3 GetIncreasedY(this Vector3 self, float value)
        {
            self.y += value;

            return self;
        }

        public static Vector3 GetIncreasedZ(this Vector3 self, float value)
        {
            self.z += value;

            return self;
        }

        public static Vector3 Create(float value) =>
            new Vector3(value, value, value);

        public static Vector3 WithX(this Vector3 self, float xValue)
        {
            self.Set(xValue, self.y, self.z);
            return self;
        }

        public static Vector3 WithY(this Vector3 self, float yValue)
        {
            self.Set(self.x, yValue, self.z);
            return self;
        }

        public static Vector3 WithZ(this Vector3 self, float zValue)
        {
            self.Set(self.x, self.y, zValue);
            return self;
        }
    }
}